...
`psychCNVassoc` is an R package developed to explore the impact of pathogenic CNVs on psychiatric disorders. It takes human Copy Number Variant (CNV) data as input, identifies genes encompassed by these CNVs, and associates them with psychiatric disorders from the PsyGeNet database. to see how the change in copy number of a gene affects crucial pathways involved in these conditions. how cnvs affect gene expression levels. The package complements existing workflows by determining whether CNVs warrant further investigation into the molecular mechanisms underlying their association with psychiatric disease comorbidities. This package accepts inputs in the format of CNV calls, which includes the chromosome number, start and end position, type of variation (deletion or duplication), which can be obtained using existing CNV detection tools like PennCNV. Additionally, it can accept a pre-defined list of genes as HGNC symbols (e.g. ‘COMT’, ‘DRD3’, 'HTR1A') for associating genes to diseases.  
...
The highlight of this package is the visualization of the most commonly observed disorders in the gene-disease association table. This representation is particularly beneficial for users as it reveals potential links between diseases and variations in copy number. Identifying these associations paves the way for deeper exploration into the molecular mechanisms underlying such conditions, ultimately contributing to a better understanding of their development and progression. This frequency of diseases is displayed as a wordcloud using the `plotDiseaseCloud` function. In the wordcloud, the larger a word is displayed, the more frequently it appears in the gene-disease association table. See `?plotDiseaseCloud` for more information, examples, and references.
...
Since the PsyGeNet database contains gene annotations for some diseases more than others, the results may be bias. Therefore, users can choose to eliminate highly dominant terms to allow viewing of other important terms. For example, the database has a greater number of gene annotations for schizophrenia, and since this is the largest term in the first iteration of the wordcloud (above), users can eliminate it as follows:

```{r error = FALSE, warning = FALSE}
plotDiseaseCloud(gene_disease_assoc, remove_most_freq = 1)
```
